    ...
    <div className="w-full">
      ...
      {filteredAssessments.length === 0 ? (
        ...
        {filteredAssessments.map((assessment) => {
          ...
          const cardClasses = `group relative flex flex-col justify-between overflow-hidden rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/50 dark:border-slate-800/50 p-8 transition-all duration-300 ${isComingSoon ? 'opacity-80' : 'hover:shadow-xl hover:shadow-emerald-500/10 hover:-translate-y-1 active:scale-[0.98] active:translate-y-0 cursor-pointer'}`;
          ...
          if (isComingSoon) {
            return (
              <div key={assessment.id} className={cardClasses}>
                {CardContent}
              </div>
            );
          }
          ...
          return (
            <Link key={assessment.id} href={AssessmentRegistry.getRoute(assessment.id)} className={cardClasses}>
              {CardContent}
            </Link>
          );
        })}
