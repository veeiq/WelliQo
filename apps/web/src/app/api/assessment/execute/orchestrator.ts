...
export class ConversationOrchestrator {
  ...
  public selectNextQuestion(
      ...
      if (!this.answeredQuestionIds.has(baselineId)) {
        ...
        if (question) {
          ...
          return {
            ...
            isNewLayer: layer !== this.currentLayer,
            newLayerName: layer,
          };
        }
      }
    ...
    const currentLayerQuestions = unasked.filter(
      (rq) => this.extractLayer(rq.question) === this.currentLayer && rq.informationGain > 10,
    );
    ...
    if (currentLayerQuestions.length > 0 && currentLayerQuestions[0]) {
      ...
      return {
        nextQuestion: currentLayerQuestions[0].question,
        isNewLayer: false,
        newLayerName: this.currentLayer,
      };
    }
