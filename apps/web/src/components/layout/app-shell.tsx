...
import { Check, Instagram, Youtube, Facebook, Twitter } from 'lucide-react';
      ...
      <footer className="w-full bg-slate-950 text-slate-400 py-16 px-4 md:px-8 text-sm leading-relaxed border-t border-slate-900 mt-auto">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8">
          ...
          <div className="lg:col-span-5 space-y-8 pr-8">
            ...
            <p className="text-slate-300 text-[17px] leading-relaxed max-w-sm font-medium">
            ...
            <div className="flex flex-col gap-2.5 text-[14px] text-slate-400">
              <div className="flex items-center gap-3"><Check className="w-4 h-4 text-emerald-500" /> Science-informed</div>
              <div className="flex items-center gap-3"><Check className="w-4 h-4 text-emerald-500" /> Personalized guidance</div>
              <div className="flex items-center gap-3"><Check className="w-4 h-4 text-emerald-500" /> Privacy-first</div>
              <div className="flex items-center gap-3"><Check className="w-4 h-4 text-emerald-500" /> No spam</div>
            </div>
        ...
        {/* CTA Section */}
        <div className="max-w-7xl mx-auto mt-20 pt-12 pb-8 border-t border-slate-800 flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left">
          <div>
            <h4 className="text-[22px] font-semibold text-white mb-2">Still thinking about your health?</h4>
            <p className="text-slate-400 text-[15px]">Take the free 5-minute assessment and get your baseline.</p>
          </div>
          <Link 
            href={AssessmentRegistry.getDirectoryRoute()}
            className="group relative flex h-12 md:h-14 items-center justify-center overflow-hidden rounded-full bg-emerald-600 px-8 md:px-10 font-medium text-white transition-all hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(16,185,129,0.2)] hover:shadow-[0_0_30px_rgba(16,185,129,0.4)]"
          >
            <span className="relative z-10 text-[15px]">Start Assessment</span>
            <span className="relative z-10 ml-2 group-hover:translate-x-1 transition-transform duration-300">→</span>
          </Link>
        </div>

        {/* Bottom Bar */}
        <div className="max-w-7xl mx-auto mt-6 pt-10 border-t border-slate-800 flex flex-col md:flex-row justify-between items-start gap-12 md:gap-8">
