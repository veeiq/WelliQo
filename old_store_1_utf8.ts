      ...
      submitQuestionnaire: async () => {
        ...
        setTimeout(() => {
          ...
          const pillars: PillarScore[] = [
            { id: 'weight', label: 'Weight Management', icon: 'ΓÜû∩╕Å', score: calcScore(bmi >= 18.5 && bmi <= 24.9 ? +20 : -15) },
            { id: 'muscle', label: 'Muscle Gain & Tone', icon: '≡ƒÆ¬', score: calcScore(isActive ? +20 : isSedentary ? -20 : 0) },
            { id: 'sleep', label: 'Sleep & Recovery', icon: '≡ƒÿ┤', score: calcScore(sleepHours >= 7 && sleepHours <= 9 ? +15 : sleepHours < 6 ? -25 : -5) },
            { id: 'nutrition', label: 'Nutrition', icon: '≡ƒÑù', score: calcScore(answers.diet_type?.includes('none') ? -10 : +5) },
            { id: 'hydration', label: 'Hydration', icon: '≡ƒÆº', score: calcScore(0) },
            { id: 'gut', label: 'Gut Health', icon: '≡ƒªá', score: calcScore(answers.gut_symptom?.includes('none') ? +15 : -15) },
            { id: 'stress', label: 'Stress & Mental Wellbeing', icon: '≡ƒÿî', score: calcScore(stressLvl <= 3 ? +15 : stressLvl >= 7 ? -25 : 0) },
            { id: 'energy', label: 'Energy Levels', icon: 'ΓÜí', score: calcScore(answers.energy_dip === 'never' ? +20 : -10) },
            { id: 'heart', label: 'Heart & Metabolic', icon: 'Γ¥ñ∩╕Å', score: calcScore(isActive && bmi < 25 ? +15 : -10) },
            { id: 'activity', label: 'Physical Activity', icon: '≡ƒÅâ', score: calcScore(isActive ? +25 : isSedentary ? -25 : +5) },
            { id: 'aging', label: 'Healthy Aging', icon: '≡ƒª┤', score: calcScore(age < 35 ? +10 : isActive ? +15 : -10) },
          ];

          if (gender === 'female') {
            pillars.push({ id: 'women', label: 'Women\'s Wellness', icon: '≡ƒæ⌐', score: calcScore(answers.cycle_status === 'regular' ? +15 : -5) });
          }
