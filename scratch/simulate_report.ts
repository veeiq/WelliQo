...
function generatePersonaReport(name: string, decisionKeywords: string[]) {
  ...
  const decisions = MasterEngineConfig.decisions.filter((d: any) => 
    decisionKeywords.some(keyword => d.clinicalMeaning.includes(keyword))
  ).map((d: any) => d.id);
...
generatePersonaReport('Office Worker with Emotional Eating', ['Desk job inactivity', 'Stress eating after work']);
...
generatePersonaReport('Middle-Aged Man with Diabetes', ['Prediabetes risk', 'Joint pain during cardio']);
